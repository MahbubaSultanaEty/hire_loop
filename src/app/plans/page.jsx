...
function PlanCard({ plan, icon: Icon }) {
  return (
    <Card
      className={`relative flex flex-col ${plan.highlighted ? "bg-purple-500/10 border-purple-500/30" : "bg-white/[0.03] border-purple-500/10"}`}
    >
      {plan.badge && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2">
          <Chip
            size="sm"
            className="bg-purple-600 text-white border-0 shadow-lg shadow-purple-500/30"
          >
            {plan.badge}
          </Chip>
        </div>
      ...
      <Card.Header className="pb-0">
        <div className="flex items-center gap-2 mb-3">
          <div
            className={`w-8 h-8 rounded-lg flex items-center justify-center ${plan.highlighted ? "bg-purple-500/20" : "bg-white/5"}`}
          >
        ...
        >
          ...
          <section>
            <button
              type="submit"
              role="link"
              className={`w-full p-2 font-medium rounded-xl transition-all ${
                plan.highlighted
      ...
      <div className="max-w-6xl mx-auto space-y-16">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-white/40 text-lg max-w-xl mx-auto">
            Whether you&apos;re looking for your next opportunity or building a
            team, we have a plan that fits.
          </p>
        ...
        <Card className="bg-purple-500/5 border-purple-500/20">
          <Card.Content className="flex items-start gap-3">
            ...
            <p className="text-white/60 text-sm leading-relaxed">
              New companies can post up to{" "}
              <span className="text-white font-medium">
                3 active jobs for free
              </span>{" "}
              — ideal for their first year of hiring — and upgrade to Growth or
              Enterprise as their hiring scales. All paid plans include a{" "}
              <span className="text-white font-medium">
                14-day money-back guarantee
              </span>{" "}
              and support upgrade/downgrade at any time with prorated billing.
            </p>
          </Card.Content>
        </Card>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {faqs.map((faq) => (
              <Card
                key={faq.q}
                className="bg-white/[0.03] border-purple-500/10"
              >
                <Card.Header>
                  <Card.Title className="text-white font-medium text-base">
                    {faq.q}
                  </Card.Title>
                </Card.Header>
